...
runner.describe('Cell Tool', () => {
  ...
  runner.it('should not crash when painting out-of-bounds', () => {
    const cases = [
      { x: -5, y: 10, name: 'negative x' },
      { x: 10, y: -5, name: 'negative y' },
      { x: 1000, y: 10, name: 'x >= width' },
      { x: 10, y: 1000, name: 'y >= height' },
    ];

    cases.forEach(({ x, y, name }) => {
      const tool = new CellTool();
      ...
      tool.paint(x, y);
      expect(canvas.setCell).toHaveBeenCalledTimes(0); // ${name}
    });
  });
